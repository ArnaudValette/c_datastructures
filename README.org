...
In the untyped implementation, *keys are treated as opaque byte sequences*.
...
For example, if a key is a struct containing pointers, you must either:
1. accept that pointer values participate in key identity,
2. or provide your own canonical serialization of the key data.
...
Keys are copied into freshly allocated memory. Depending on the key policy you choose to enforce with the untyped implementation, you may be confronted with two problems.
